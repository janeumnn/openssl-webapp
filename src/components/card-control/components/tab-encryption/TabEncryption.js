...
function TabEncryption({ runCommand }) {
  ...
  const isOutputFile = useRef(false);
  const [outputFile, setOutputFile] = useState(null);

  useEffect(() => {
    if (isOutputFile.current) {
      setOutputFile(state.outputFile);
      isOutputFile.current = false;
    }
  }, [state.outputFile]);
        ...
        <>
          ...
          <Form.Row>
            <Form.Group as={Col} lg={'auto'} md={'auto'}>
              <label for="enc-output-file">Output file</label>
              <InputGroup>
                <Form.Control id="enc-output-file" value={outputFile.name} disabled />
                <InputGroup.Append>
                  <Button variant="outline-secondary" onClick={downloadOutputFile}>
                    Download
                  </Button>
                </InputGroup.Append>
              </InputGroup>
